...
namespace TradeConsole
{
    class BinanceRequest
    {
        ...
        {
            string arg = $"batchOrders=%5B%7B%22symbol%22%3A+%22{orders.OrderBuy.Symbol}%22%2C+%22side%22%3A+%22BUY%22%2C+%22type%22%3A+%22LIMIT%22%2C+%22quantity%22%3A+%22{orders.Quantity}%22%2C+%22price%22%3A+%22{orders.OrderBuy.Price}%22%2C+%22timeInForce%22%3A+%22GTC%22%7D%2C+%7B%22symbol%22%3A+%22{orders.OrderSell.Symbol}%22%2C+%22side%22%3A+%22SELL%22%2C+%22type%22%3A+%22LIMIT%22%2C+%22quantity%22%3A+%22{orders.Quantity}%22%2C+%22price%22%3A+%22{orders.OrderSell.Price}%22%2C+%22timeInForce%22%3A+%22GTC%22%7D%5D&timestamp=" + DateTimeOffset.Now.ToUnixTimeMilliseconds();
        ...
        public static void MakeOrder(ref Order order, bool isBuy)
        {
            string type;
            if (isBuy)
            {
            ...
            string arg = $"batchOrders=%5B%7B%22symbol%22%3A+%22{order.Symbol}%22%2C+%22side%22%3A+%22{type}%22%2C+%22type%22%3A+%22LIMIT%22%2C+%22quantity%22%3A+%22{order.Quantity}%22%2C+%22price%22%3A+%22{order.Price}%22%2C+%22timeInForce%22%3A+%22GTC%22%7D%5D&timestamp=" + DateTimeOffset.Now.ToUnixTimeMilliseconds();
            RestClient client = new(ApiSettings.BaseUrl);
            RestRequest request = new(ApiSettings.BatchOrdersUrl + "?" + arg + "&signature=" + Tools.HashSHA256(Encoding.UTF8.GetBytes(arg), Encoding.UTF8.GetBytes(ApiSettings.SecretKey)), Method.POST);
            request.AddHeader("X-MBX-APIKEY", ApiSettings.ApiKey);
            var response = client.Execute(request);
            ...
            Tools.Log(response.Content);
            var jsonData = JsonConvert.DeserializeObject<Structure.BatchOrders>("{\"OrderArray\":" + response.Content + "}");
            order.Id = jsonData.OrderArray[0].OrderId;
        }
        ...
        public static void MakeCancel(Order order)
        {
            string arg = $"symbol={order.Symbol}&orderId={order.Id}&timestamp=" + DateTimeOffset.Now.ToUnixTimeMilliseconds();
            RestClient client = new(ApiSettings.BaseUrl);
            RestRequest request = new(ApiSettings.OrderUrl + "?" + arg + "&signature=" + Tools.HashSHA256(Encoding.UTF8.GetBytes(arg), Encoding.UTF8.GetBytes(ApiSettings.SecretKey)), Method.DELETE);
            request.AddHeader("X-MBX-APIKEY", ApiSettings.ApiKey);
            var response = client.Execute(request);
            Console.WriteLine(response.Content);
            Tools.Log(response.Content);
